...
class Agent:
    ...
    def train_model(self, states0, states1, actions, rewards):
        length = len(actions)
        self.q_targets =[]
        prediction = self.model.predict(np.reshape(states1, (-1, 27)))
        for n in range (length):                
            action_mask = np.array([1, 1, 1, 1])
            action_mask = np.logical_xor(action_mask, actions[n])
            q_target = actions[n]*(rewards[n]+0.99*prediction[np.argmax(prediction[n])])
            self.q_targets.append(q_target)

    
        #Train the model 
        self.model.fit(np.asarray(np.reshape(states0, (-1, 27))), np.asarray(self.q_targets), batch_size=32, )
